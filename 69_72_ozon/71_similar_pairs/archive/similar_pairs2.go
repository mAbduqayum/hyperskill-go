...
func solveTestCase() {
	...
	patternCache = make(map[string]StringPattern)
...
func getPattern(s string) StringPattern {
	...
	pattern := StringPattern{string(even), string(odd)}
...
func areSimilar(pattern1, pattern2 StringPattern) bool {
	if len(pattern1.odd) == 0 || len(pattern2.odd) == 0 {
		return pattern1.even == pattern2.even
	}
	return pattern1.even == pattern2.even || pattern1.odd == pattern2.odd
}
